...
## 이메일 발송 절대 원칙
- CEO(jacob@08liter.com) 승인 없이는 단 1통도 발송 불가
- 승인 방법: jacob@08liter.com으로 검수 이메일 발송 → 회신으로만 승인
- 대시보드 승인 버튼은 DB수집만 승인 / 이메일 발송은 반드시 이메일 회신으로만
- 검수 이메일 필수 포함: 발송개요 + 샘플 3건 전문 + 예상결과 + 비용
- 회신 "승인" → 전체 발송 / "수정: [내용]" → 수정 후 재검수 / "취소" → 발송 취소
